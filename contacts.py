...
def show_all_contacts():
    if not contacts:
        messagebox.showinfo("All Contacts", "No contacts to display.")
        return

    contact_list = ""
    for name, details in contacts.items(): #all the contact details in dictionary
        contact_list += f"Name: {name}\n"
        for contact in details:
            contact_list += f"  Phone: {contact['phone']}\n"
            contact_list += f"  Address: {contact['post_code']}\n"
            contact_list += f"  Email: {contact['email'] if contact['email'] else 'N/A'}\n\n"
